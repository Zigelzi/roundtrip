...
Milestones 01 to 03 are merged. 01 (packing list): create trips (departure + return date or duration), list them upcoming first, and add/remove items per family member on a trip page, with items added in place via HTMX, earlier names suggested, one item per name per member. 02 (pack items): a separate packing page where items are ticked packed per member, with a per-member filter, progress and a collapsed packed section. 03 (family items): a "Family" bucket that owns shared items (sunscreen, first aid kit) alongside the four people, stored as an out-of-range `family_member` row (id 100, the `familyBucketID` constant in `cmd/web/app.go`) so `FAMILY_NAMES` positional renaming can never reach it. Milestone 0 (walking skeleton) proved the stack before all of them. Next up is 04, activity planning; `spec/activity-catalogue.md` drafts its content and its decisions.

Structure mirrors go-tiimit: entrypoint in `cmd/web` (`main.go`, `app.go`, `view/` templ templates), SQLite access in `internal/db` (sqlc-generated), Goose migrations + sqlc queries under `sql/`.
...
- After each implementation slice, append a row to `spec/allocation-log.md` recording whether it ran in the main session or a sub-agent, and what it cost. This is an open experiment; the log is how it gets settled.
