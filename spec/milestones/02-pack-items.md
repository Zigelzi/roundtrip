...
4. Marking an item packed, and unpacking it again, because things get taken back out.
...
Known implementation hazard (red-team): a native `<details>` for the packed section loses its open state on every HTMX swap, which S3 forbids. Resolve it with `hx-preserve`, a swap that leaves the wrapper alone, or Alpine, with Alpine only if the lighter options fail.
