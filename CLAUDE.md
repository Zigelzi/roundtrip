...
- **Commits are the agent's job (standing authorization — commit without asking).** Human-PM makes only the initial baseline commit. Milestone = branch off `main`; each vertical slice = one focused commit once its test is green; merge to `main` only after Human-PM accepts (step 7).
