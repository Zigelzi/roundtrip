...
5. Main session implements milestone by milestone as vertical slices, sequentially, each slice satisfying its BDD acceptance test. Parallel dev sub-agents only if slices provably share no files. A task is right-sized if one agent can implement and self-verify it in one context without re-loading the whole app. Each slice is committed to the milestone branch once its test is green.
   ...
   - **Size before spawning.** Under Option B, a slice of a few dozen lines in one or two files runs inline in the main session: a spawn's fixed start-up cost (about 40k tokens) is several times the work itself (04 slice 2: 43k for 15 lines). The main session still writes the tests before the code, so the tests stay a contract written first, though by the same author. At this repo's size a spawn starts to pay off for slices of roughly 100+ hand-written lines.
   - **Visual bugs: measure before the second fix.** When a defect from the phone check is visual (animation, rendering, timing) and the first fix does not resolve it, stop fixing by reasoning. Reproduce it in a headless browser, with screenshots and pixel comparisons at phone pixel density, before changing more code. The main session does this inline; it is not a new spawn point. How: [`../docs/visual-debugging.md`](../docs/visual-debugging.md). Milestone 04's edit transition took four reasoning-only rounds, each uncovering the next problem; one measured pass found the real cause.
